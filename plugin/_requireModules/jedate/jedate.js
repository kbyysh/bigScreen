Cube("/_requireModules/jedate/jedate.js", ["/node_modules/jquery/dist/jquery.js"], function (module, exports, require, load, process, global) {
  ...
  (function (window, factory) {
    ...
    var $Q = function $Q(selector, content) {
      content = content || document;
      return selector.nodeType ? selector : content.querySelector(selector);
    };
    var jeDate = function jeDate(elem, options) {
    ...
    jet.extend(jet, {
      ...
      equals: function equals(arrA, arrB) {
        if (!arrB) return false;
        if (arrA.length != arrB.length) return false;
        for (var i = 0, l = arrA.length; i < l; i++) {
          if (_instanceof(arrA[i], Array) && _instanceof(arrB[i], Array)) {
      ...
      docScroll: function docScroll(type) {
        type = type ? 'scrollLeft' : 'scrollTop';
        return document.body[type] | document.documentElement[type];
      },
      docArea: function docArea(type) {
      ...
      getDaysNum: function getDaysNum(y, m) {
        var num = 31,
      ...
      prevMonth: function prevMonth(y, m, n) {
        return jet.getYM(y, m, 0 - (n || 1));
      },
      nextMonth: function nextMonth(y, m, n) {
        return jet.getYM(y, m, n || 1);
      },
      setCss: function setCss(elem, obj) {
        for (var x in obj) elem.style[x] = obj[x];
      },
      html: function html(elem, _html) {
        return typeof _html === 'undefined' ? elem && elem.nodeType === 1 ? elem.innerHTML : undefined : typeof _html !== 'undefined' && _html == true ? elem && elem.nodeType === 1 ? elem.outerHTML : undefined : elem.innerHTML = _html;
      },
      text: function text(elem, value) {
        var innText = document.all ? 'innerText' : 'textContent';
        return typeof value === 'undefined' ? elem && elem.nodeType === 1 ? elem[innText] : undefined : elem[innText] = value;
      },
      val: function val(elem, value) {
      ...
      getDateTime: function getDateTime(obj) {
    ...
    jet.extend(jeDatePick.prototype, {
      init: function init() {
        ...
        that.minDate = '';
        that.maxDate = '';
        if (!isShow || !trigges) ymarr = that.getCurrValue();
        if (!isShow || !trigges) {
      ...
      renderDate: function renderDate() {
        ...
        if (jet.isObj(opts.theme)) {
          ...
          styleDiv.innerHTML = stCell + WC + ';}' + stCell + ' .jedate-header em{' + WC + ';}' + stCell + ' .jedate-content .yeartable td.action span,' + stCell + ' .jedate-content .monthtable td.action span,' + stCell + ' .jedate-content .yeartable td.action span:hover,' + stCell + ' .jedate-content .monthtable td.action span:hover{' + BG + ';border:1px ' + t.bgcolor + ' solid;' + WC + ';}' + stCell + ' .jedate-content .daystable td.action,' + stCell + ' .jedate-content .daystable td.action:hover,' + stCell + ' .jedate-content .daystable td.action .lunar,' + stCell + ' .jedate-header,' + stCell + ' .jedate-time .timeheader,' + stCell + ' .jedate-time .hmslist ul li.action,' + stCell + ' .jedate-time .hmslist ul li.action:hover,' + stCell + ' .jedate-time .hmslist ul li.disabled.action,' + stCell + ' .jedate-footbtn .timecon,' + stCell + ' .jedate-footbtn .btnscon span{' + BG + ';' + WC + ';}' + stCell + ' .jedate-content .daystable td.other,' + stCell + ' .jedate-content .daystable td.other .nolunar,' + stCell + ' .jedate-content .daystable td.other .lunar{' + OTH + '}' + stCell + ' .jedate-content .daystable td.contain,' + stCell + ' .jedate-content .daystable td.contain:hover{background-' + OTH + '}';
        ...
        $('.jedate-menu p').removeClass('jedate-menu-select');
        switch (that.format) {
          case 'YYYY-MM-DD':
            $('.jedate-menu p:nth-of-type(1)').addClass('jedate-menu-select');
            break;
          case 'YYYY-MM':
            $('.jedate-menu p:nth-of-type(2)').addClass('jedate-menu-select');
            break;
          case 'YYYY':
            $('.jedate-menu p:nth-of-type(3)').addClass('jedate-menu-select');
            break;
          case 'YYYY-MM-DD hh:mm:ss':
            $('.jedate-menu p:nth-of-type(4)').addClass('jedate-menu-select');
            break;
          case 'YYYY-MM-DD hh:mm':
            $('.jedate-menu p:nth-of-type(5)').addClass('jedate-menu-select');
            break;
          case 'YYYY-MM-DD hh':
            $('.jedate-menu p:nth-of-type(6)').addClass('jedate-menu-select');
            break;}

      },
      getCurrValue: function getCurrValue() {
        var that = this,
        mats = jet.reMatch(that.format),
        range = that.$opts.range,
        isEmpty = that.getValue() != '',
        isShow = jet.isBool(that.$opts.isShow),
        curVal = [],
        ...
        if (isEmpty && isShow) {
          ...
          jet.each(new Array(range ? 2 : 1), function (a) {
            ...
            jet.each(jet.reMatch(getVal[a]), function (i, val) {
              !!mats[i] && (curVal[a][mats[i]] = parseInt(val));
            });
            that.dlen >= 2 && !curVal[a]['MM'] && (curVal[a]['MM'] = 1);
            that.dlen >= 3 && !curVal[a]['DD'] && (curVal[a]['DD'] = 1);
          });
      ...
      setValue: function setValue(fnStr, matStr, bool) {
        var that = this,
      ...
      getValue: function getValue(valobj) {
        var that = this,
        valCell = that.valCell,
        opts = that.$opts,
        reObj,
      ...
      storeData: function storeData(curr, next) {
        next = next || {};
        var that = this,
        opts = that.$opts,
        multi = opts.multiPane,
      ...
      dateTemplate: function dateTemplate() {
        ...
        var yaerHtml = '<table class="yeartable year{%= i==0 ? "left":"right"%}" style="display:{%=year ? "block":"none"%};"><tbody><tr>' + '{% for(var y=0;y<=11;y++){ %}<td class="{%=yearlist[i][y].style%}" @on="yearClick({%=yearlist[i][y].y%})"><span>{%=yearlist[i][y].y%}' + ytxt + '</span></td>{% if((y+1)%3==0){ %} </tr>{% } %} {% } %} </tbody></table>';
        var monthHtml = '<table class="monthtable month{%= i==0 ? "left":"right"%}" style="display:{%=month ? "block":"none"%};"><tbody><tr>' + '{% for(var m=0;m<=11;m++){ %}<td class="{%=monthlist[i][m].style%}" ym="{%=monthlist[i][m].y%}-{%=monthlist[i][m].m%}" @on="monthClick({%=monthlist[i][m].y%}-{%=monthlist[i][m].m%})"><span>{%=monthlist[i][m].m%}' + mtxt + '</span></td>{% if((m+1)%3==0){ %} </tr>{% } %} {% } %} </tbody></table>';
        var daysHtml = '<table class="daystable days{%= i==0 ? "left":"right"%}" style="display:{%=day ? "block":"none"%};"><thead><tr>' + '{% for(var w=0;w<lang.weeks.length;w++){ %} <th>{%=lang.weeks[w]%}</th> {% } %}</tr></thead><tbody>' + '<tr>{% for(var d=0;d<=41;d++){ %}<td class="{%=daylist[i][d].style%}" ymd="{%=daylist[i][d].ymd%}" @on="daysClick({%=daylist[i][d].ymd%})">{%=daylist[i][d].day%}</td>{% if((d+1)%7==0){ %} </tr>{% } %} {% } %} </tbody></table>';
        var hmsHtml = '<div class="jedate-time">{% for(var h=0;h<timelist.length;h++){ %}<div class="timepane"><div class="timeheader">{%= timelist.length == 1 ? lang.timetxt[0]:lang.timetxt[h+1]%}</div><div class="timecontent">' + '<div class="hmstitle"><p>{%=lang.times[0]%}</p><p>{%=lang.times[1]%}</p><p>{%=lang.times[2]%}</p></div>' + '<div class="hmslist">{% for(var t=0;t<3;t++){ %}<div class="hmsauto"><ul>{% for(var s=0;s<timelist[h][t].length;s++){ %}<li class="{%=timelist[h][t][s].style%}" @on="hmsClick({%= h %},{%= h>0?3+t:t %})">{%= timelist[h][t][s].hms < 10 ? "0" + timelist[h][t][s].hms :timelist[h][t][s].hms %}</li>{% } %}</ul></div>{% } %}</div></div>' + '</div>{% } %}</div>';
        var shortHtml = opts.shortcut.length > 0 ? '{% for(var s=0;s<shortcut.length;s++){ %}<p @on=shortClick({%= shortcut[s].name %})>{%=shortcut[s].name%}</p>{% } %}' : '';
        var ymtitHtml = function () {
          ...
          if (that.dlen == 1) {
            ...
            ymtitStr = '<span class="ymbtn" @on="monthShow({%=daytit[i].MM%})">{%=daytit[i].MM%}' + mtxt + '</span>' + '<span class="ymbtn" @on="yearShow({%=daytit[i].YYYY%})">{%=daytit[i].YYYY%}' + ytxt + '</span>';
          }
          return ymtitStr;
        }();
        ...
        var paneHtml = '{% for(var i=0;i<pane;i++){ %}<div class="jedate-pane">' + '<div class="jedate-header">{% if(i==0){ %}' + ymButton[0] + '{% }else{ %}' + ymButton[1] + '{% } %}' + ymtitHtml + '</div>' + '<div class="jedate-content{%= i==1?" bordge":"" %}">' + YMDStr + '</div>' + '</div>{% } %}';
        var btnStr = '{% if(timebtn){%}<div class="timecon" style="cursor: pointer;" @on="timeBtn">{%=lang.timetxt[0]%}</div>{% } %}<div class="btnscon"><span class="close" @on="closeBtn">取消</span>{% if(clear){ %}<span class="clear" @on="clearBtn">{%=lang.clear%}</span>{% } %}{% if(today){ %}<span class="today" @on="nowBtn">{%=lang.today%}</span>{% } %}{% if(yes){ %}<span class="setok" @on="sureBtn">{%=lang.yes%}</span>{% } %}</div>';
      ...
      compileBindNode: function compileBindNode(dom) {
        var self = this,
        aton = '@on';
        var acquireAttr = function acquireAttr(atVal) {
          var args = /\(.*\)/.exec(atVal);
          if (args) {
            args = args[0];
      ...
      methodEventBind: function methodEventBind() {
        var that = this,
        opts = that.$opts,
        multi = opts.multiPane,
        DTS = that.$data,
        ...
        jet.extend(that, {
          yearBtn: function yearBtn(type, val) {
            var yarr = val.split('#'),
          ...
          yearShow: function yearShow(val) {
          ...
          monthBtn: function monthBtn(type, val) {
          ...
          monthShow: function monthShow(val) {
            DTS.year = false;
          ...
          shortClick: function shortClick(val) {
            var btnCfg = {
              日期: {
                format: 'YYYY-MM-DD',
                dlen: 3 },
            ...
            if (that.format !== btnCfg[val].format) {
              that.$opts.format = btnCfg[val].format;
              that.format = btnCfg[val].format;
              that.dlen = btnCfg[val].dlen;
              that.closeDate();
              that.minDate = jet.isType(that.$opts.minDate, 'function') ? that.$opts.minDate(that) : that.$opts.minDate;
              that.maxDate = jet.isType(that.$opts.maxDate, 'function') ? that.$opts.maxDate(that) : that.$opts.maxDate;
              var data = that.getCurrValue();
              that.storeData(data[0], data[1]);
              that.renderDate();
            }
          },
          yearClick: function yearClick(val) {
          ...
          daysClick: function daysClick(val) {
            if (jet.hasClass(this, 'disabled')) return;
          ...
          hmsClick: function hmsClick(idx, num) {
      ...
      eachYear: function eachYear(val, type) {
        var that = this,
        opts = that.$opts,
        yNum = parseInt(val),
        yarr = [],
      ...
      eachMonth: function eachMonth(val, type) {
        var that = this,
        opts = that.$opts,
        range = opts.range,
        marr = [],
      ...
      eachDays: function eachDays(yd, md, ds, idx) {
        var that = this,
        count = 0,
        daysArr = [],
        opts = that.$opts,
        ...
        var regExpDate = function regExpDate(date, cls) {
      ...
      eachTime: function eachTime(hmsArr, type) {
        var that = this,
        opts = that.$opts,
        range = opts.range,
        multi = opts.multiPane,
      ...
      parseValue: function parseValue(fnObj, matStr) {
        var that = this,
        valArr = [],
        opts = that.$opts,
        range = opts.range;
      ...
      dateRegExp: function dateRegExp(valArr) {
      ...
      showFestival: function showFestival() {
        var that = this,
        opts = that.$opts;
      ...
      lunarOrien: function lunarOrien(obj, self, pos) {
        var tops,
        leris,
        ortop,
        orleri,
      ...
      dateOrien: function dateOrien(elbox, valCls, pos) {
        var that = this,
        tops,
        leris,
        ortop,
      ...
      locateScroll: function locateScroll() {
        var that = this,
